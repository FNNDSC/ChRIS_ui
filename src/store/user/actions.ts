...
export const setAuthTokenSuccess = (auth: { token: string; username: string }) =>
  action(UserActionTypes.SET_TOKEN_SUCCESS, auth) // NOTE: To be done: Save user token to cookie or session
export const setUserLogout = (username: string) => action(UserActionTypes.LOGOUT_USER, username)
