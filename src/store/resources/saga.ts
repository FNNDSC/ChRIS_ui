...
import { FileBrowserPathFileList, PluginInstance, FileBrowserPath } from '@fnndsc/chrisapi'
...
function* watchGetPluginStatusRequest() {
  yield takeEvery(ResourceTypes.GET_PLUGIN_STATUS_REQUEST, pollInstanceEndpoints)
}

function* watchResetActiveResources() {
  yield takeEvery(ResourceTypes.RESET_ACTIVE_RESOURCES, handleResetActiveResources)
}

function* watchSelectedPlugin() {
  yield takeEvery(PluginInstanceTypes.GET_SELECTED_PLUGIN, pollorCancelEndpoints)
}
