...
import { FaRedo, FaQuestionCircle, FaCodeBranch, FaCubes, FaEye } from 'react-icons/fa'
...
export const QueryResults: React.FC<QueryResultsProps> = ({
          ...
          <CardHeader onExpand={expandPatient.bind(PatientCard)}>
            <Grid hasGutter style={{ width: '100%' }}>
              ...
              <GridItem lg={4}>
                ...
                <div>
                  <b>DoB</b> <Moment format="MMMM Do YYYY">{`${PatientBirthDate}`}</Moment>
                </div>
              </GridItem>
              ...
              <GridItem lg={4} style={{ textAlign: 'right', color: 'gray' }}>
                <div>
                  <b>
                    {patient.studies.length} {pluralize('study', patient.studies.length)}
                  </b>
                </div>
                <div>
                  Latest on {LatestDate(patient.studies.map((s) => s.StudyDate)).toDateString()}
                </div>
              </GridItem>
            </Grid>
          </CardHeader>
  ...
  const StudyCard = ({ study }: { study: PACSStudy }) => {
    ...
    const StudyActions = () => {
      ...
      useEffect(() => {
        if (cubeHasStudy || !pullStatus || !pullStatus.isRunning) return () => clearTimeout(poll)
        ...
        if (pullStatus.isRunning) setPoll(setTimeout(() => _poll().then(setPullStatus), 1000))
        // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      if (cubeHasStudy)
        return (
          <div style={{ color: 'gray' }}>
            <Button variant="link" style={{ padding: 0 }} onClick={() => route(cubeStudyPath)}>
              <b>Browse</b>
            </Button>
          ...
          <>
            {seriesFiles.length && (
              <div style={{ marginTop: '-1em', wordWrap: 'break-word' }}>
                <FileDetailView
                  preview="small"
                  selectedFile={seriesFiles[Math.floor(series.NumberOfSeriesRelatedInstances / 2)]}
                />
                ...
                {openSeriesPreview && (
                  ...
                  >
                    <FileDetailView
                      selectedFile={
                        seriesFiles[Math.floor(series.NumberOfSeriesRelatedInstances / 2)]
                      }
                      preview="large"
                    />
                  </Modal>
