...
export const handleUploadedFiles = (
  ...
  if (uploadedFolders.length > 0) {
    dispatch(setSearchedFolders(uploadedFolders, path));
    dispatch(setCurrentPathSearch([path], "uploads"));
  }
};
...
export const handleFeedFiles = (
  ...
  const feedFolders: { name: string; path: string }[] = [];
...
export const handlePacsFiles = (pacsFiles: any[], dispatch: React.Dispatch<any>) => {
  ...
  dispatch(setSearchedFolders(pacsFolders, "services"));
