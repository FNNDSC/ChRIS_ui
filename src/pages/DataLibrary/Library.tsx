...
import { RouterContext, RouterProvider } from "../Routing/RouterContext";
...
export const Library: React.FC = () => {
  ...
  const actions = {
    ...
    isSeriesSelected: (s: Series) =>
      s.every((f) => state.selectData.includes(f)),
    select: (item: File | Series) => {
      if (Array.isArray(item))
        setState({ selectData: [...state.selectData, ...item] });
      else setState({ selectData: [...state.selectData, item] });
    },

    clear: (itemid?: File | Series) => {
      if (!itemid) setState({ selectData: [] });
      else {
        ...
        if (Array.isArray(itemid)) {
          ...
          for (const id of itemid) selection = fselection(selection, id);
          setState({ selectData: selection });
        } else setState({ selectData: fselection(state.selectData, itemid) });
      }
    },

    createFeedWithSelected: router.actions.createFeedWithData.bind(
      Library,
      state.selectData
    ),
  };
    ...
    <>
      ...
      {state.selectData.length !== 0 && (
        <AlertGroup isToast>
          <Alert
            variant="info"
            title={`Selected ${state.selectData.length} ${pluralize(
              "file",
            ...
            actionLinks={
              <React.Fragment>
                <AlertActionLink onClick={actions.createFeedWithSelected}>
                  Create Feed
                </AlertActionLink>
                <AlertActionLink
                  onClick={actions.clear.bind(Library, undefined)}
                >
                  Clear
                </AlertActionLink>
              </React.Fragment>
