...
import { fromReadonlyArray, ReadonlyNonEmptyArray } from "fp-ts/ReadonlyNonEmptyArray";
 ...
 */
function findVisualDatasetInstancePairs(plinsts: readonly PluginInstance[]): Pick<VisualDataset, "dataPlinst" | "indexPlinst">[] {
  return plinsts
...
function isPlVisualDataset(plinst: PluginInstance): boolean {
  ...
  return plugin_name === 'pl-visual-dataset'
    && isCompatibleVersion(plugin_version);
}

function isCompatibleVersion(pluginVersion: string): boolean {
  return constants.COMPATIBLE_PL_VISUAL_DATASET_VERSIONS
    .findIndex((v) => v === pluginVersion) !== -1
}

function flattenVisualDatasetsReturn(x: readonly GetDatasetsResult[]): GetDatasetsResult {
  return pipe(
    fromReadonlyArray(x),
    OptionMatch(
 ...
 */
function reduceListOfObjects<T extends {[key: string]: object[]}>(x: ReadonlyNonEmptyArray<T>): T {
  const acc = Object.fromEntries(Object.keys(x[0]).map((key) => [key, [] as object[]]));
  for (const ele of x) {
