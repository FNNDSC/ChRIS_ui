...
const Status = () => {
  ...
  if (pluginStatus && pluginStatus.length > 0) {
    return (
      <>
        <Steps className="node-details__status" direction="horizontal" size="small">
        ...
        <Steps direction="horizontal" size="small" className="node-details__status-descriptions">
          {pluginStatus.map((label: any, index: number) => (
            <Step
              key={index}
              description={label.description}
              status={label.status === true ? 'finish' : label.error === true ? 'error' : undefined}
            />
          ))}
        </Steps>
      </>
