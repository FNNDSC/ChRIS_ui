...
import { Button, Grid, GridItem, ExpandableSection } from '@patternfly/react-core'
...
import { FaDownload, FaTerminal, FaCalendarAlt, FaWindowClose } from 'react-icons/fa'
...
const NodeDetails: React.FC<INodeProps> = ({ expandDrawer }) => {
  ...
  const command = React.useCallback(getCommand, [plugin, instanceParameters, pluginParameters])
  ...
  const Time = (
    <>
      <FaCalendarAlt />
      <Moment format="DD MMM YYYY @ HH:mm">{selectedPlugin.data.start_date}</Moment>
    </>
  )
  return (
    <div className="node-details">
      ...
      <Grid className="node-details__grid">{renderGridItem('Status', <StatusTitle />)}</Grid>
      ...
      >
        <Grid className="node-details__grid">
          {renderGridItem('Selected Node ID', <span>{selectedPlugin.data.id}</span>)}
          {renderGridItem(
            'Plugin',
            <span style={{ fontFamily: 'monospace' }}>
              {selectedPlugin.data.plugin_name}, ver {selectedPlugin.data.plugin_version}
            </span>
          )}
          ...
          {runTime && (
            <>
              {renderGridItem(
                'Total Runtime',
                <span>{selectedPlugin && selectedPlugin.data && runTime(selectedPlugin)}</span>
              )}
            </>
          )}
          {cancelled &&
      ...
      <div style={{ marginTop: '1.25em' }}>{download.error && download.error}</div>
    </div>
  )
}
