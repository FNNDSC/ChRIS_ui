...
const Tree = (props: {
  ...
  handleNodeClick: (nodeName: number, pipelineId: number, plugin_id: number) => void
}) => {
  ...
  const { pluginPipings, pipelinePlugins } = state.pipelineData[currentPipelineId]
...
const LinkData: React.FC<LinkProps> = ({ linkData, orientation }) => {
  ...
  const applyOpacity = (opacity: number, transitionDuration: number, done = () => null) => {
