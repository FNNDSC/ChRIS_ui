...
function assignInternalProperties(data: Datum[], currentDepth = 0) {
  ...
  return d.map((n) => {
    ...
    if (nodeDatum.children && nodeDatum.children.length > 0) {
      nodeDatum.children = assignInternalProperties(nodeDatum.children, currentDepth + 1)
    }
    return nodeDatum
  })
}
...
function findNodesById(nodeId: string, nodeSet: TreeNodeDatum[], hits: TreeNodeDatum[]) {
  if (hits.length > 0) {
    return hits
  }
...
const FeedTree = (props: AllProps) => {
  ...
  const { feedTreeProp, currentLayout } = useTypedSelector((state) => state.feed)
  ...
  const generateTree = () => {
    ...
    const d3Tree = tree<TreeNodeDatum>()
      ...
      .separation((a, b) =>
        a.data.parentId === b.data.parentId ? separation.siblings : separation.nonSiblings
      )
    ...
    if (data) {
      const rootNode = d3Tree(hierarchy(data[0], (d) => (d.__rd3t.collapsed ? null : d.children)))
      nodes = rootNode.descendants()
      links = rootNode.links()

      const targetNodes = links.filter(
        (link) =>
          // @ts-ignore
  ...
  return (
    <div className={`feed-tree grabbable mode_${mode === false ? 'graph' : 'tree'}`} ref={divRef}>
      <div className="feed-tree__container">
        <div className="feed-tree__container--labels">
          ...
          {mode === false && (
            <div className="feed-tree__orientation">
              <Alert variant="info" title="You are now in a ts node selection mode" />
            </div>
          )}
        </div>
        {!props.isSidePanelExpanded && (
          <div className="feed-tree__container--panelToggle">
            <div className="feed-tree__orientation">
              <Button type="button" onClick={() => props.onExpand('side_panel')}>
                Node Panel
              </Button>
            </div>
          </div>
        ...
        <svg className={`${svgClassName}`} width="100%" height="85%">
          ...
          >
            {links?.map((linkData, i) => (
              <Link orientation={orientation} key={`link${i}`} linkData={linkData} />
            ))}
            ...
            {nodes?.map(({ data, x, y, parent }, i) => (
              <NodeWrapper
                ...
                toggleLabel={feedState.toggleLabel}
                overlayScale={
                  feedState.overlayScale.enabled ? feedState.overlayScale.type : undefined
                }
              />
            ))}
          </TransitionGroupWrapper>
      ...
      {!props.isBottomPanelExpanded && (
        <div className="feed-tree__container--panelToggle">
          <div className="feed-tree__orientation">
            <Button type="button" onClick={() => props.onExpand('bottom_panel')}>
              Feed Browser
            </Button>
          </div>
        </div>
...
export default React.memo(FeedTree, (prevProps: AllProps, nextProps: AllProps) => {
  if (
