...
useEffect(() => {
    ...
    if (!!treeRef.current && !!props.items && props.items.length > 0) {
      ...
      const activeNode = items.filter((node) => {
        return node.data.status === "started" || node.data.status==='scheduled'
...
}, [props.items, props.selected, props.pluginStatus]);
...
export default connect(mapStateToProps, null)(FeedTree);
