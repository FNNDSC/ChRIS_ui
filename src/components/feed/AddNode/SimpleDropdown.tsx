...
import { Dropdown, DropdownToggle, DropdownItem, Banner } from '@patternfly/react-core'
...
const SimpleDropdown: React.FC<SimpleDropdownProps> = ({
  ...
  const [paramFlag, value, type, placeholder, paramName] = unPackForKeyValue(dropdownInput[id])
  ...
  const handleInputChange = (e: any) => {
    handleChange(id, paramFlag, e.target.value, type, placeholder, false, paramName)
  }
    ...
    <>
      <div className="plugin-configuration">
        <Dropdown
          onSelect={onSelect}
          toggle={
            <DropdownToggle id="toggle-id" onToggle={onToggle} toggleIndicator={AiFillCaretDown}>
      ...
      {type === 'boolean' && (
        <Banner variant="info">
          Boolean flags don&apos;t require the user to implicitly provide values.
        </Banner>
      )}
    </>
  )
