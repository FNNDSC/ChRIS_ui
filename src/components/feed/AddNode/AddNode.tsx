...
import { InfrastructureIcon, CloseIcon, CodeBranchIcon } from '@patternfly/react-icons';
...
class AddNode extends React.Component<AddNodeProps, AddNodeState> {
  ...
  async convertData() {
    ...
    const transformedNodes = await Promise.all(nodes.map(this.convertPluginInstance));
  ...
  generateFooter() {
    ...
    return step === 0 ?
      ...
      (
        <>
          <Button variant="link" onClick={this.handleModalClose}>Cancel</Button>
