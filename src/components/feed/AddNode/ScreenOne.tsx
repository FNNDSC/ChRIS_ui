...
import { Dropdown, FormGroup, DropdownToggle, DropdownItem } from '@patternfly/react-core';
...
class ScreenOne extends React.Component<ScreenOneProps, ScreenOneState> {
  ...
  constructor(props: ScreenOneProps) {
    ...
    this.handleParentDropdownToggle = this.handleParentDropdownToggle.bind(this);
  ...
  generateTypeDropdown() {
    const pluginItems = [
      <DropdownItem key={0} isHovered>Plugin</DropdownItem>,
      <DropdownItem key={1} isDisabled>Pipeline</DropdownItem>
    ]
    const toggle = <DropdownToggle onToggle={this.handleTypeDropdownToggle}>Plugin</DropdownToggle>;
