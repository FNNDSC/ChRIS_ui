...
function parseHtml(paramString: string, params: PluginParameter[]): [string, string[]] {
  ...
  for (const token of tokens) {
    ...
    if (!validParam) {
      str = `${str.substring(0, token.index + offset)}<span class="warning">${token[0]}</span>${str.substring(token.index + offset + token[0].length)}`;
      ...
      if (value && value.match(dirRegex) === null) {
        str = `${str.substring(0, token.index + offset + flag.length + 1)}<span class="warning">${value}</span>${str.substring(token.index + offset + flag.length + value.length + 1)}`; // the one is for the space or equals sign
        offset += '<span class="warning"></span>'.length;
        errors.push(`The value provided for '${paramName}' is not a valid path.`);
      }
    }
  }
