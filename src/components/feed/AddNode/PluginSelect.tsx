...
import { Accordion, AccordionItem, AccordionContent, AccordionToggle, TextInput } from '@patternfly/react-core';
...
class PluginList extends React.Component<PluginListProps, PluginListState> {
  ...
  matchesFilter(plugin: Plugin) {
    return plugin.data.name.toLowerCase().trim().includes(this.state.filter.toLowerCase().trim());
  }
  ...
  render() {
    ...
    const loading = new Array(3).fill(null).map((_, i) => (
...
class PluginSelect extends React.Component<PluginSelectProps, PluginSelectState> {
  ...
  async fetchRecentPlugins() {
    ...
    while (pluginIds.length < amount && pluginInstanceList.hasNextPage) {
      ...
      const pluginsInstances = pluginInstanceList
        .getItems()
        .filter((pluginInst: PluginInstance, i, instances: PluginInstance[]) => {
    ...
    const plugins = await Promise.all(pluginIds.map(id => {
