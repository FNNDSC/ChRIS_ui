...
class ScreenTwo extends React.Component<ScreenTwoProps, ScreenTwoState> {
  ...
  generateDefaultParamString(params: PluginParameter[]) {
    return params.map(param => `${param.data.flag} ${param.data.default}`).join(' ');
  }
  ...
  render() {
    ...
    return (
      <div className="screen-two">
        ...
        >
          {
            this.state.params.filter(param => param.data.ui_exposed).map(param => {
              return (
                <div key={param.data.id} className="param-item">
                  <b className="param-title">[{param.data.flag} &lt;{param.data.name}&gt;]</b>
