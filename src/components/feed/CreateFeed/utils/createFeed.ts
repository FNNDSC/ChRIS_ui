          ...
          if (
            ...
            nodes_info.forEach((node) => {
              if (computeEnvs && computeEnvs[node['piping_id']]) {
                const compute_node = computeEnvs[node['piping_id']]['currentlySelected']

                const title = pipeline.title && pipeline.title[node['piping_id']]
                if (title) {
                  node.title = title
                }
                if (compute_node) {
  ...
  if (selectedPlugin) {
    ...
    try {
      ...
      if (fsPlugin instanceof Plugin) {
        const data = await getRequiredObject(dropdownInput, requiredInput, fsPlugin)
...
export const getRequiredObject = async (
  ...
  const params: PluginParameter[] = await fetchResource<PluginParameter>(paginate, boundFn)
...
function generatePathForLocalFile(data: CreateFeedData) {
  ...
  const normalizedFeedName = data.feedName.toLowerCase().replace(/ /g, '-').replace(/\//g, '')
