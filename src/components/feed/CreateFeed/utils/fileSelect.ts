...
export const getNewTreeData = (
  ...
  const loop = (data: DataBreadcrumb[]) => {
    data.forEach((item) => {
      if (curKey.indexOf(item.key as string) === 0 || (item.key as string).indexOf(curKey) === 0) {
        if (item.children) {
          loop(item.children)
...
export const generateTreeNodes = async (treeNode: EventDataNode): Promise<DataBreadcrumb[]> => {
  const arr: any[] = []
  //@ts-ignore
