...
export async function fetchResources(pipelineInstance: Pipeline) {
  ...
  const pipelinePlugins: any[] = await fetchResource(params, boundPipelinePluginFn)
...
export const generatePipelineWithName = async (pipelineName: string) => {
  ...
  const pipelineInstance: Pipeline = await client.getPipeline(pipelineInstanceId)
...
export async function fetchComputeInfo(plugin_id: number, dictionary_id: number) {
  const client = ChrisAPIClient.getClient()
