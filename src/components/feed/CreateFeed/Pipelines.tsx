...
import { Tree, ConfigurationPage, UploadJson, GeneralCompute } from '../Pipelines'
import { fetchComputeInfo, generatePipelineWithName, fetchPipelines } from './utils/pipelines'
...
const Pipelines = () => {
  ...
  const handleNodeClick = async (nodeName: number, pipelineId: number, plugin_id: number) => {
  ...
  const handleDispatch = (result: UploadJsonProps) => {
    const { pipelineInstance, parameters, pluginPipings, pipelinePlugins } = result
    dispatch({
        ...
        <DataList aria-label="pipeline list">
          {pipelines.length > 0 &&
            pipelines.map((pipeline) => (
              <DataListItem
                isExpanded={!!(expanded && expanded[pipeline.data.id])}
              ...
              >
                <DataListItemRow>
                  <DataListToggle
                    ...
                    onClick={async () => {
                      if (
                        ...
                        const { resources } = await generatePipelineWithName(pipeline.data.name)
                  ...
                  <DataListItemCells
                    dataListCells={[
                      <DataListCell key={pipeline.data.name}>
                        <div className="plugin-table-row" key={pipeline.data.name}>
                  ...
                  >
                    ...
                    >
                      {selectedPipeline === pipeline.data.id ? 'Deselect' : 'Select'}
                    </Button>
                    <Button
                      key="delete-action"
                      onClick={async () => {
                        const filteredPipelines = pipelines.filter(
                          (currentPipeline: any) => currentPipeline.data.id !== pipeline.data.id
                        )
                        dispatch({
                          type: Types.SetPipelines,
                          payload: {
                ...
                >
                  ...
                  state.pipelineData[pipeline.data.id] ? (
                    <>
                      ...
                      <ConfigurationPage pipeline={pipeline} currentPipelineId={pipeline.data.id} />
                    </>
                  ) : (
