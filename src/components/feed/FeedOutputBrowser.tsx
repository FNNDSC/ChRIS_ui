...
function convertFiles(files: FeedFile[], selected: IPluginItem): IFeedFile[] {
  return files.map(file => {
    ...
    return {
      ...
      fname: file.data.fname.split(`${selected.plugin_name}_${selected.id}`)[1].slice(1),
    }
  });
}
...
class FeedOutputBrowser extends React.Component<FeedOutputBrowserProps, FeedOutputBrowserState> {
  ...
  componentDidUpdate(prevProps: FeedOutputBrowserProps) {
    ...
    if (!prevProps.selected || prevProps.selected.id !== selected.id && !files) {
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  setSelectedFile: (file: IUITreeNode, folder: IUITreeNode) => dispatch(setSelectedFile(file, folder))
})
