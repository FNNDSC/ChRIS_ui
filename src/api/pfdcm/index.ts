...
class PFDCMClient {
  ...
  constructor({}: PFDCMClientOptions = PFDCMDefaultOptions) {
    if (!process.env.REACT_APP_PFDCM_URL) throw Error('PFDCM URL is undefined.')
  ...
  private async __find(query: PFDCMFilters = {}) {
    if (!this.service) throw Error('Set the PACS service first, before querying.')
  ...
  async status(query: PFDCMFilters = {}): Promise<PFDCMPull> {
    ...
    try {
      ...
      if (images.requested !== 0) pullstatus.attempts = DEFAULT_STAGE_ATTEMPTS_MUL(images.requested)
...
export class PFDCMPull {
  ...
  equals(pull: PFDCMPull): boolean {
    if (pull.query == this.query && pull.stage === this.stage && pull.progress === this.progress)
      return true
  ...
  get isRunning() {
    return this.stage !== PACSPullStages.COMPLETED && this.stage !== PACSPullStages.NONE
  }
